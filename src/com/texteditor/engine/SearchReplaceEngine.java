...
public class SearchReplaceEngine {
	public enum SearchOption {CASE_SENSITIVE, WHOLE_WORDS_ONLY, USE_REGEX}
	private Set<SearchOption> options;
	public SearchReplaceEngine() { this.options = new HashSet<>();}
	...
	public List<Integer> findAll(String text, String searchTerm) {
		...
		try {
			...
			while (m.find()) { results.add(m.start());}
		} catch (PatternSyntaxException e) {
            // Invalid regex pattern
			if (!options.contains(SearchOption.USE_REGEX)) { results = findAllLiteral(text, searchTerm);}
		}
		return results;
	}
	...
	private String buildPattern(String searchTerm) {
		...
		if (options.contains(SearchOption.WHOLE_WORDS_ONLY)) { pattern.append("\\b");}
	...
	public String replaceFirst(String text, String searchTerm, String replacement) {
        	...
        	int end = start + getMatchLength(text, searchTerm, start);
	...
	private int getMatchLength(String text, String searchTerm, int position) { return text.substring(position).indexOf(searchTerm) + searchTerm.length();}
}
